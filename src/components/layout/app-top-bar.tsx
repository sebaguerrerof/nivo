...
export function AppTopBar() {
  ...
  return <header className="sticky top-0 z-30 hidden h-[4.5rem] border-b border-[var(--border-subtle)] bg-[color:var(--canvas)/0.88] backdrop-blur-xl lg:block"><div className="mx-auto flex h-full max-w-[100rem] items-center justify-between px-8 xl:px-10"><p className="text-[15px] font-semibold tracking-[-0.02em] text-[var(--foreground)]">{getCurrentSectionTitle(location.pathname)}</p><div className="flex items-center gap-1"><StreakIndicator streak={gamification.data?.streak} /><XpIndicator summary={gamification.data} /><NotificationButton /><span aria-hidden="true" className="mx-1 h-6 w-px bg-[var(--border-subtle)]" /><UserMenu /></div></div></header>
}