...
import { AlertCircle, CalendarClock, Pause, Play, Plus } from 'lucide-react'
...
import { useOverduePayments, usePaymentActions, usePaymentOccurrences, useRecurringPayments, useUpcomingPayments } from '@/features/payments/hooks/use-payments'
...
export function PaymentsPage() {
  ...
  const upcoming = useUpcomingPayments(user?.id, timezone, 3)
  const overdue = useOverduePayments(user?.id, timezone)
  ...
  const pending = useMemo(() => monthPayments.filter((payment) => !['paid', 'skipped', 'overdue'].includes(getPaymentOccurrenceStatus(payment, today))), [monthPayments, today])
  ...
  const overduePayments = overdue.data ?? []
