...
export function MonthlyHeatmap({ heatmap, startDate, endDate }: MonthlyHeatmapProps) {
  ...
  return (
    <div>
      ...
      <div aria-live="polite" className="mt-5 rounded-[var(--radius-control)] bg-[var(--surface-subtle)] px-4 py-3 text-sm leading-6 text-[var(--foreground-muted)]">{getHeatmapDescription(selectedDay, selectedDate)}</div>
    </div>
  )
}